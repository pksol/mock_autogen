...
def copy_result_to_clipboard(func):
    ...
    def to_clipboard(*args, **kwargs):
        ...
        try:
            ...
            logger.warning("Could not copy func results to clipboard", exc_info=True)
        return result
...
def safe_travels(warnings: list, action: str, source_code: str):
    """
    Takes care of calling `self.generic_visit(node)` and allows the inner method to focus on the logic. Including in the case of exception.

    If an exception does happen, a warning in the form of
    `f"# could not {action} on node: {str(node)}"` would be added to warnings, and the tree travel would continue to the next node.

    Args:
        warnings: a list to hold all warnings
        action: the name of the action
        source_code: the matching source code we are parsing, used for better warnings
